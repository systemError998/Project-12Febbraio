<?php 
/**
 * SKT Filmmaker functions and definitions
 *
...
function skt_filmmaker_setup() {
	$GLOBALS['content_width'] = apply_filters( 'skt_filmmaker_content_width', 640 );
...
* Enqueue theme fonts.
*/
function skt_filmmaker_fonts() {
	$fonts_url = skt_filmmaker_get_fonts_url();

	// Load Fonts if necessary.
	if ( $fonts_url ) {
		require_once get_theme_file_path( 'inc/wptt-webfont-loader.php' );
		wp_enqueue_style( 'skt-filmmaker-fonts', wptt_get_webfont_url( $fonts_url ), array(), '20201110' );
	}
}
add_action( 'wp_enqueue_scripts', 'skt_filmmaker_fonts', 1 );
add_action( 'enqueue_block_editor_assets', 'skt_filmmaker_fonts', 1 );

/**
 * Retrieve webfont URL to load fonts locally.
 */
function skt_filmmaker_get_fonts_url() {
	$font_families = array(
	...
	return apply_filters( 'skt_filmmaker_get_fonts_url', add_query_arg( $query_args, 'https://fonts.googleapis.com/css' ) );
}
...
function skt_filmmaker_scripts() {
	...
	wp_enqueue_script( 'skt-filmmaker-customscripts', get_stylesheet_directory_uri() . '/js/custom.js', array('jquery') );
...
define('SKT_FILMMAKER_SKTTHEMES_THEME_URI', trailingslashit( esc_url( get_template_directory_uri() ) ) );
 ...
 */
function skt_filmmaker_custom_excerpt_length( $length ) {
...
function skt_filmmaker_activation_notice(){
    ...
    <div class="notice notice-info is-dismissible"> 
        <div class="skt-filmmaker-notice-container">
        	<div class="skt-filmmaker-notice-img"><img src="<?php echo esc_url( SKT_FILMMAKER_SKTTHEMES_THEME_URI . 'images/icon-skt-templates.png' ); ?>" alt="<?php echo esc_attr('SKT Templates');?>" ></div>
            <div class="skt-filmmaker-notice-content">
            <div class="skt-filmmaker-notice-heading"><?php echo esc_html__('Thank you for installing SKT Filmmaker!', 'skt-filmmaker'); ?></div>
            <p class="largefont"><?php echo esc_html__('SKT Filmmaker comes with 150+ ready to use Elementor templates. Install the SKT Templates plugin to get started.', 'skt-filmmaker'); ?></p>
            </div>
            <div class="skt-filmmaker-clear"></div>
        </div>
    </div>
    <?php
}
...
add_action( 'wp_enqueue_scripts', 'skt_filmmaker_custom_enqueue_wc_cart_fragments' );
function skt_filmmaker_custom_enqueue_wc_cart_fragments() {
    wp_enqueue_script( 'wc-cart-fragments' );
}

add_filter( 'woocommerce_add_to_cart_fragments', 'skt_filmmaker_mini_cart_count');
function skt_filmmaker_mini_cart_count($fragments){
    ob_start();
    ?>
...
add_filter( 'woocommerce_add_to_cart_fragments', 'skt_filmmaker_refresh_cart_total');
function skt_filmmaker_refresh_cart_total( $fragments ) {
		ob_start();
	?>
		<div id="mini-cart-total">
			<?php echo esc_html_e('Total', 'skt-filmmaker'); ?>
				<div class="clear"></div>
